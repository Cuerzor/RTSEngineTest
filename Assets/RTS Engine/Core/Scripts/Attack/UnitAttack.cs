...
namespace RTSEngine.EntityComponent
{
    public class UnitAttack : FactionEntityAttack
    {
        ...
        protected override bool MustStopProgress()
        {
            ...
            else
            {
                    ...
                    && Target.instance.CanMove() && Target.instance.MovementComponent.HasTarget)
                {
                    ...
                    if(IsInTargetRange
                        && Vector3.Distance(unit.transform.position, RTSHelper.GetAttackTargetPosition(Target)) > Mathf.Max(followDistance, initialEngagementDistance))
                    {
        ...
        protected override void OnEnterTargetRange()
        {
            ...
            initialEngagementDistance = Vector3.Distance(unit.transform.position, RTSHelper.GetAttackTargetPosition(Target));
        }
